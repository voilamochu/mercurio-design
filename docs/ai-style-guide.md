...
The primary subject (structure, terrain formation, or colony) should occupy the centre 40–60 % of the frame. Never dead-centre. Offset slightly to the left or right to leave room for the planet type icon overlay.
...
4. **Star fields should be consistent across all space backgrounds.** Same star colour distribution, same density profile, same nebula treatment. Only the central subject or nebula colour changes per region.
...
Space backgrounds for planet cards should be neutral enough to not compete with the planet artwork. Use a subtle dark variant with reduced star density behind the artwork region.
...
The asset compiler composes cards from independent layers. The following stack defines the compositing order from bottom to top.
...
Layer 1:  Planet Artwork
          ─────────────────
Layer 2:  Frame (border, header divider, footer bar)
          ─────────────────
Layer 3:  Card Content (planet type, VP, hitboxes, resource icons)
          ─────────────────
Layer 4:  Runtime Overlays (ownership bar, status badges, selection glow)
          ─────────────────
Layer 5:  Text (card name, rules text — where applicable)
```
...
|---|---|---|---|
| Background | AI-generated raster | Asset pipeline | No |
| Artwork | AI-generated raster | Asset pipeline | No |
| Frame | SVG template | `generate-frame.js` | No |
| Content | Icon/SVG library | Template compiler | No |
| Overlays | Code | Rendering engine | Yes |
| Text | Font rendering | Rendering engine | Yes |

### 7.2 Independence Rules

- No layer should depend on the pixel content of any layer below it.
- The Frame layer must not assume specific artwork colours or composition.
- The Artwork layer must not extend beyond its defined region.
- Runtime overlays must not be baked into exported card assets.
- Background and Artwork are interchangeable as long as they conform to the region bounds defined in `slots.json`.
