...
const PATHS = {
  sourceDir: path.join(ROOT, 'generated', 'cards-tech'),
  modelFile: path.join(ROOT, 'generated', 'models', 'technologies.json'),
...
function build() {
  ...
  if (!fs.existsSync(PATHS.sourceDir)) {
    ...
    console.error('Run `npm run build:tech-cards` first.');
    process.exit(1);
  }
  ...
  const modelTechs = modelRaw.technologies;
  ...
  const modelDst = path.join(PATHS.dataDir, 'technologies.json');
  fs.copyFileSync(PATHS.modelFile, modelDst);

  if (fs.existsSync(PATHS.manifestFile)) {
    const manifest = JSON.parse(fs.readFileSync(PATHS.manifestFile, 'utf-8'));
    manifest.technologyCount = modelTechs.length;
    manifest.files.technologies = 'data/technologies.json';
    fs.writeFileSync(PATHS.manifestFile, JSON.stringify(manifest, null, 2) + '\n', 'utf-8');
  }

  const errors = [];

  if (cardPaths.length !== EXPECTED_TECH_CARDS) {
    errors.push(`Expected ${EXPECTED_TECH_CARDS} SVGs, found ${cardPaths.length}`);
  }

  const exportedIds = cardPaths.map(f => path.basename(f, '.svg')).sort();
  const modelIds = modelTechs.map(t => t.assetId).sort();
  ...
  console.log('\n\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500');
  console.log('  TECH BGA EXPORT REPORT');
  console.log('\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500');
  console.log(`  Tech SVGs:       ${exportedCount}`);
  console.log(`  Tech data:       technologies.json`);
